...
CXXFLAGS_ = -Wno-unused-command-line-argument -I/usr/local/include $(CXXFLAGS)

TOOLCHAIN_ = CMAKE_CXX_FLAGS=$(CXXFLAGS_)
...
static_FLAGS  += CMAKE_CXX_COMPILER=$(CXX);CMAKE_C_COMPILER=$(CC);CMAKE_BUILD_TYPE=Release;$(TOOLCHAIN);$(CONFIG);BUILD_SHARED_LIBS=OFF;STATIC_BUILD=ON
CONFIG_SHARED = $(CONFIG);BUILD_SHARED_LIBS=OFF
